...
public class StdJDBCDelegate implements DriverDelegate, StdJDBCConstants {
    ...
    /**
     * @see org.quartz.impl.jdbcjobstore.DriverDelegate#selectPausedTriggerGroups(Connection)
     */
    ...
    @Override
    public int insertJobLog(Connection conn, CronTrigger trigger, long start, long end, String ip) throws SQLException {
        PreparedStatement ps = null;
        try {
            ps = conn.prepareStatement(rtp(INSERT_JOB_EXECUTE_LOGS));
            JobKey jobKey = trigger.getJobKey();
            TriggerKey triggerKey=trigger.getKey();
            if (jobKey == null||triggerKey==null) {
                return 0;
            }
            ps.setString(1, triggerKey.getGroup());
            ps.setString(2, jobKey.getName());
            ps.setLong(3, start);
            ps.setLong(4, end);
            ps.setString(5, ip);
