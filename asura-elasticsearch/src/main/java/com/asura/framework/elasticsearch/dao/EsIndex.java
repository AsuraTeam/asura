...
public class EsIndex implements IEsIndexDao {
    ...
    public boolean isAliasExists(String indexName, String alias) {
        TransportClient client = esClientFactory.getClient();
        IndicesAdminClient indicesAdminClient = client.admin().indices();
        AliasesExistResponse response = indicesAdminClient.prepareAliasesExist(esClientFactory.getIndexs(alias)).addIndices(esClientFactory.getIndexs(indexName)).get();
        return response.isExists();
    }

    public boolean addAliasIndex(String indexName, String alias) {
        TransportClient client = esClientFactory.getClient();

        IndicesAdminClient indicesAdminClient = client.admin().indices();
        IndicesAliasesResponse response = indicesAdminClient.prepareAliases().addAlias(esClientFactory.getIndexs(indexName), esClientFactory.getIndexs(alias)).get();

        return response.isAcknowledged();
    }

    public boolean removeAliasIndex(String indexName, String alias) {
        TransportClient client = esClientFactory.getClient();

        IndicesAdminClient indicesAdminClient = client.admin().indices();
        IndicesAliasesResponse response = indicesAdminClient.prepareAliases().removeAlias(esClientFactory.getIndexs(indexName), esClientFactory.getIndexs(alias)).get();
    ...
    public AnalyzeResponse analyze(String indexName,String keyWords,String tokenizerName) {
        TransportClient client = esClientFactory.getClient();
        IndicesAdminClient indicesAdminClient = client.admin().indices();
        return indicesAdminClient.prepareAnalyze(esClientFactory.getIndexs(indexName),keyWords).setAnalyzer(tokenizerName).get();
    }
