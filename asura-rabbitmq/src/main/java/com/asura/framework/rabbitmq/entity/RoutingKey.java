...
public class RoutingKey extends NameKey {
    ...
    public String getKey() throws AsuraRabbitMqException {
        ...
        if (!RegExpUtil.isNameOfLettersOrUnderline(this.getSystem())) {
            throw new AsuraRabbitMqException("RoutingKey:system name must match [A-Za-z0-9_]+,current is:" + this.getSystem());
        }

        if (!RegExpUtil.isNameOfLettersOrUnderline(this.getModule())) {
            throw new AsuraRabbitMqException("RoutingKey:module name must match [A-Za-z0-9_]+,current is:" + this.getModule());
        }

        if (!RegExpUtil.isNameOfLettersOrUnderline(this.getFunction())) {
            throw new AsuraRabbitMqException("RoutingKey:function name must match [A-Za-z0-9_]+,current is:" + this.getFunction());
        }
