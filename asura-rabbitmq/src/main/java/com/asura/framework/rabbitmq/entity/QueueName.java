...
public class QueueName extends NameKey {
    ...
    public String getName() throws AsuraRabbitMqException {
        ...
        if (!RegExpUtil.isNameOfLettersOrUnderline(this.getSystem())) {
            throw new AsuraRabbitMqException("QueueName:system name must match [A-Za-z0-9_]+，current is :" + this.getSystem());
        }

        if (!RegExpUtil.isNameOfLettersOrUnderline(this.getModule())) {
            throw new AsuraRabbitMqException("QueueName:module name must match [A-Za-z0-9_]+，current is :" + this.getModule());
        }

        if (!RegExpUtil.isNameOfLettersOrUnderline(this.getFunction())) {
            throw new AsuraRabbitMqException("QueueName:function name must match [A-Za-z0-9_]+，current is :" + this.getFunction());
        }
