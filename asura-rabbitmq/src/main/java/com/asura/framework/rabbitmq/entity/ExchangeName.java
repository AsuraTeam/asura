...
public class ExchangeName extends QueueName {
    ...
    public String getName() throws AsuraRabbitMqException {
        ...
        if (!RegExpUtil.isNameOfLettersOrUnderline(this.getSystem())) {
            throw new AsuraRabbitMqException("ExchangeName:system name must match [A-Za-z0-9_]+,current is:" + this.getSystem());
        }

        if (!RegExpUtil.isNameOfLettersOrUnderline(this.getModule())) {
            throw new AsuraRabbitMqException("ExchangeName:module name must match [A-Za-z0-9_]+,current is:" + this.getModule());
        }

        if (!RegExpUtil.isNameOfLettersOrUnderline(this.getFunction())) {
            throw new AsuraRabbitMqException("ExchangeName:function name must match [A-Za-z0-9_]+,current is:" + this.getFunction());
        }
