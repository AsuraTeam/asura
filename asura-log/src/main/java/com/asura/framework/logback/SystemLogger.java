...
public class SystemLogger {
	...
	public Object doBasicProfiling(final ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			// 记录耗时
			logger.debug("{}, 耗时：{} ms, 进入aop到执行完耗时：{} ms", logJson, (end - start), (end - startAll));
			...
			if (null != returnType && String.class.isAssignableFrom(returnType)){
				return dto.toJsonString();
			}else if (null != returnType && DataTransferObject.class.isAssignableFrom(returnType)){
				return dto;
			}else{
				throw e;
			}
			...
			MDC.remove(METHOD);
			tran.complete();
			logger.info("{},catId:{},接入cat后整体耗时: {} ms",Cat.getCurrentMessageId(), logJson, (endAll - startAll));
		}
	}
